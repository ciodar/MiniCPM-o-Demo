...
# ============ 构建文档静态资源 ============
# /docs served by gateway reads from static/docs/, generated by docs-app.
# Set SKIP_DOCS_BUILD=1 only when editing backend code without docs changes.
if [ -f "$PROJECT_DIR/docs-app/package.json" ]; then
    if [ "${SKIP_DOCS_BUILD:-0}" = "1" ]; then
        echo "[Docs] Skipping docs build (SKIP_DOCS_BUILD=1)"
    else
        echo "[Docs] Building docs-app -> static/docs ..."
        if command -v bun >/dev/null 2>&1; then
            (cd "$PROJECT_DIR/docs-app" && bun run build)
        else
            echo "[Docs] ERROR: bun is required to build docs-app"
            exit 1
        fi
